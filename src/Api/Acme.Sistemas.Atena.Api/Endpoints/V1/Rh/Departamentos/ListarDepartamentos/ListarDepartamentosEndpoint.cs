...
public sealed class ListarDepartamentosEndpoint : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapGet("/api/v1/rh/departamentos", async (
            int? skip,
            int? take,
            IMediator mediator,
            CancellationToken cancellationToken) =>
        {
            var request = new ListarDepartamentosRequest(skip ?? 0, take ?? 50);
        ...
        .Produces<ListarDepartamentosResponse>();
    }
}
